...
export class DocumentWorkflow extends WorkflowEntrypoint<Env,DocumentParams>{
 async run(event:WorkflowEvent<DocumentParams>,step:WorkflowStep):Promise<{decisionKey:string}|void>{
  ...
  try{
   ...
   const readerKey=run.mode==='batch'?await batchReader(runner,pack,readerRequest):await runner.vendor(readerRequest,pack,raw=>decodeReader(raw,pack.pins.reader,pack.typeFile.types.map(type=>type.id),upload.fullText,undefined,pack.settings.readerEvidencePolicy));
