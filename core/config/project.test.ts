...
test('Luna6 recovery uses verified standard pricing even when the reader uses Batch',()=>{for(const mode of ['interactive','batch']as const){assert.equal(generic.prices[mode].recovery.inputNanodollarsPerMillion,'100000000');assert.equal(generic.prices[mode].recovery.outputNanodollarsPerMillion,'500000000');}});
