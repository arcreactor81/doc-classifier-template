...
Expand **View recorded evidence** beside a document to see the saved opinions and exact reader quotes. Missing validated output is shown explicitly. Evidence does not download a manifest or close the run.
...
## 4. Download the manifest and build folders

When every document has an outcome, select **Download manifest and close run**. Save the JSON file. This also closes the run and removes its uploaded text/outline/full document state. Decisions, vendor outputs and the manifest remain. You can then work with your local originals.

Open **Build folders**:

1. Choose the saved **manifest JSON**.
2. Choose the **source folder** containing the originals.
3. Choose a **destination folder**, preferably a new folder for this run, and allow writing.
4. Enter the destination's full path so path lengths can be checked. Keep the naming defaults unless a path warning asks for shorter names.
...
Open **Corrections**, choose the same manifest, then choose the corrected tree. Mark only the folders you actually checked and select **Review corrections**. Changing the selected run or manifest requires choosing the tree again.
