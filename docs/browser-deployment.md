...
Have a GitHub or GitLab account, a Cloudflare account with the required services enabled, and your own OpenAI and TypeSafe API keys. Approve the account's Cloudflare billing requirements in its dashboard. Cloudflare infrastructure charges are separate from the application's model-spending ceiling.
...
Open /health. Resolve its numbered blockers: project definitions, owner-approved budget, model configuration and verified token counting. The generic pack deliberately contains no invented taxonomy or spending authorization. A coding harness is optional for editing the project pack; GitHub's web editor can commit changes.
