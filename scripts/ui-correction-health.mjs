...
try{
 ...
 await page.getByLabel('Run identifier').fill('run1');await page.getByRole('button',{name:'Choose corrected tree',exact:true}).click();
 ...
 async function load(runId){const choice=page.waitForEvent('filechooser');await page.getByRole('button',{name:'Choose manifest JSON',exact:true}).click();await(await choice).setFiles({name:'manifest.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(manifest(runId)))});}
 await load('run1');await page.getByRole('button',{name:'Choose corrected tree',exact:true}).click();await page.getByText('Files in corrected tree: 1',{exact:true}).waitFor();checks++;
 ...
 await page.getByRole('button',{name:'Choose corrected tree',exact:true}).click();await page.getByText('Files in corrected tree: 1',{exact:true}).waitFor();await load('run2');await page.getByRole('button',{name:'Review corrections',exact:true}).click();await page.waitForTimeout(100);assert.equal(posts.length,1);checks++;
 ...
 await page.getByRole('button',{name:'Choose corrected tree',exact:true}).click();await page.getByLabel('Run identifier').fill('run3');await page.evaluate(()=>window.releasePicker());await page.waitForTimeout(100);
 assert.equal(await page.getByText('Files in corrected tree: 1',{exact:true}).count(),0);checks++;
