...
try{
 ...
 await page.getByRole('link',{name:'Corrections',exact:true}).click();const choice=page.waitForEvent('filechooser');await page.getByRole('button',{name:'Choose results file'}).click();await(await choice).setFiles({name:'manifest.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(result.manifest))});
 await page.getByRole('button',{name:'Choose corrected output folder'}).click();await page.getByText('Documents in the corrected folder: 2',{exact:true}).waitFor();await page.getByRole('checkbox',{name:'type_b',exact:true}).check();await page.getByRole('button',{name:'Review corrections',exact:true}).click();await page.waitForTimeout(100);
