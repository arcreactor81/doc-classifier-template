...
try{
const page=await browser.newPage();const posts=[];let changePack=false,uploadHeld=false,releaseUpload;
...
await page.route('**/api/runs/created/documents',async r=>{if(!uploadHeld){uploadHeld=true;await new Promise(resolve=>{releaseUpload=resolve;});}await r.fulfill({json:{accepted:true}});});
...
async function setup(){
 ...
 await page.evaluate(async ({pack,base})=>{
  ...
  document.querySelector('#budget-harness')?.remove();const host=document.createElement('section');host.id='budget-harness';const mode=document.createElement('select');const option=document.createElement('option');option.value='interactive';mode.append(option);host.append(mode);document.querySelector('main').append(host);
...
console.log('Budget confirmation: 15 checks passed; real local preparation without a tokenizer, fixture APIs only, no vendor calls.');
