...
### Follow-up source selection repair (2026-09-24)
Owner found missing-source error after saving comparison labels. Browser reproduction established stale path records selected through the globally shared extraction pointer; folder scanning also included nested generated output. Added tab-scoped extraction identity and per-extraction frozen reference mapping, preserving existing records and server links. New reference gets a fresh extraction context. Button now says Choose files for the comparison run; Home explains saved-feedback context. Generated output is detected by exact summary filename/header and excluded only after explicit user choice; selecting generated root asks for originals. Genuine resume mismatch has visible explicit restart action and never silently retries. No files moved/deleted, labels unchanged. Nine new unit tests; complete gates380private/344public plus existing runtime checks pass. Real Edge OPFS picker/hash/DOCX-worker regression combines stale state, cross-tab interference, output exclusion and explicit genuine-resume restart; verifies retained original/copy bytes, old records/reference, zeroAPIwrites/vendorcalls. Public and private repair publication follows; live user retry remains to verify.
...
Completedwaitandpartialdispatchrepair integrated: durableabsolute deadlines, elapsedknownresetguardrecheck, completedcallbackACKproof, partialreservationgenerationfence009, read-onlyinplacestatus/actionableerrors. Combinedlocalworkerd/D1/R2faultsequence79assertionswithonesyntheticrequestandzerorealvendorcalls;controlledpendingHTTPbrowsertestshowsprogresswithoutautomaticcontinuation. Published008migrationunchanged. No live run mutated byagent.


### Combined lifecycle release checks (2026-09-24)
Threeparallelfixtracksintegrated. Private446/public410unit tests pass; combinedactuallocalworkerd/D1/R2failuresequence79assertions passes alongsideprior46/17/38runtimechecks. ControlledpendingHTTPbrowsertestprovesin-place schedulingcounts whilePOSTisstillpending; consentremainsrequired, noautomaticPOST. TestsusesyntheticWorkflowcontrol/clock/vendoroutbound, notactualplatformevictionorclassificationquality. No livegeneration2orretrytriggered. Current113paidrecordsremainpreserved. Public/privatepublicationincludesabsolute-wait,partialplan009fenceandactionableprogresscopy; broadervisualreworkstilldeferred.
