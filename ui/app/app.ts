...
function intro(main: HTMLElement, title: string, lede?: string): void { const head=el('div',undefined,'page-intro');if(page==='home')head.append(el('p',c.workspaceEyebrow,'eyebrow'));head.append(el('h1', title)); if (lede) head.append(el('p', lede, 'lede'));main.append(head); }
function blockers(main: HTMLElement): void { if (!health || health.status === 'READY') return; const box = section(c.setup, c.setupDetail);box.classList.add('setup-card'); const list = el('ol'); for (const blocker of health.blockers) { const item = el('li'); item.append(el('strong', blocker.headline), el('p', blocker.action), technical({ code: blocker.code, details: blocker.details })); list.append(item); } box.append(list); main.append(box); }
...
async function home(main: HTMLElement): Promise<void> {
  ...
  const note = el('p', c.browserReason, 'privacy-banner');note.prepend(icon('file'));main.append(note); intro(main, c.hero, c.lede);const steps=el('ol',undefined,'workflow-steps');c.flowSteps.forEach((step,index)=>{const item=el('li');item.append(el('span',String(index+1).padStart(2,'0'),'step-number'));const text=el('div');text.append(el('strong',step.title),el('p',step.detail));item.append(text);steps.append(item);});main.append(steps);
  const grid = el('div', undefined, 'grid'); const work = section(c.local, c.localDetail); const side = section(c.preflight, c.pendingEstimate);work.classList.add('source-card');side.classList.add('preflight-card');
...
async function runs(main: HTMLElement, generation: number): Promise<void> {
  ...
  if (!activeRun) { const data = await api<{ runs: Json[] }>('/api/runs'); if (generation !== renderId) return; if (!data.runs.length) main.append(el('p',c.noRuns,'empty')); for (const run of data.runs) { const box = section(String(run.id));box.classList.add('run-card'); const stats = el('div',undefined,'stats'); stat(stats,c.status,String(run.status)); stat(stats,c.progress,`${run.completed} / ${run.total}`); if(Number(run.unaccountedCalls)>0)box.append(el('p',c.unaccountedSpend+': '+run.unaccountedCalls));box.append(stats,button(c.nav.runs,()=>route('runs',String(run.id)))); main.append(box); } return; }
...
async function correct(main: HTMLElement): Promise<void> {
  intro(main,c.correct,c.correctLede);if(!supported){main.append(el('p',c.browserReason));return;}const box=section(c.nav.correct,c.noAutomatic);const run=field(box,c.correctionRun,'text',activeRun||loadedManifest?.runId||'');const choices=el('div',undefined,'actions');const correctionStatus=el('div');const output=el('div');const folders=el('fieldset');folders.append(el('legend',c.checked));let listing:Json[]=[];let sidecarPaths:string[]=[];const checked=new Map<string,HTMLInputElement>();
