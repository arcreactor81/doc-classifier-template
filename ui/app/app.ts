...
async function runs(main: HTMLElement, generation: number): Promise<void> {
  ...
  const run = data.run; const box = section(`${c.runId}: ${activeRun}`); const stats = el('div',undefined,'stats'); stat(stats,c.status,String(run.status)); stat(stats,c.progress,`${run.completed} / ${run.total}`); box.append(stats);
  ...
  const spendBox=section(Number(run.unaccountedCalls)>0?c.knownSpend:c.spend,c.spendingLag);
